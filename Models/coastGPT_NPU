...
from . import DEFAULT_IMAGE_TOKEN
try:
    from .qwen_language_model import QwenLanguageModel  # 可选：Qwen/MoE 语言模块
except Exception:
    QwenLanguageModel = None
...
class CoastGPT(nn.Module):
    def __init__(self, config: ml_collections.ConfigDict):
        ...
        # 语言处理模块：根据 text.path 判定是否使用 Qwen/MoE
        use_qwen = False
        try:
            text_path = getattr(config.text, "path", "")
            use_qwen = isinstance(text_path, str) and ("qwen" in text_path.lower())
        except Exception:
            use_qwen = False

        if use_qwen and QwenLanguageModel is not None:
            logger.info("Using QwenLanguageModel based on text.path")
            self.language = QwenLanguageModel(config)
        else:
        ...
        self.moe_aux_weight = float(moe_cfg.get("aux_weight", 0.0))
        self.moe_router_entropy_coef = float(moe_cfg.get("router_entropy_coef", 0.0))
        self.moe_router_zloss_coef = float(moe_cfg.get("router_zloss_coef", 0.0))

        # --- [SEG] 指令支持：在分词器中注册特殊标记 ---
        self.seg_token_str = "[SEG]"
        try:
            # 注册并扩展词表
            num_added = self.language.tokenizer.add_tokens([self.seg_token_str], special_tokens=True)
            if num_added and hasattr(self.language, "get_text_encoder"):
                self.language.get_text_encoder().resize_token_embeddings(len(self.language.tokenizer))
            self.seg_token_id = self.language.tokenizer.convert_tokens_to_ids(self.seg_token_str)
        except Exception:
            # 兼容非标准 tokenizer 情况；仍支持字符串检测
    ...
    def forward(self, data: Dict):
        ...
        if self.physics_enabled and isinstance(self.vision, DualVisionEncoder):
            # DualVisionEncoder 内部包含 global 和 local 两个分支
        ...
        # --- 深度加固：视觉输出保护 ---
        # 如果视觉编码器输出本身就不稳定，需要在这里强制转 fp32 处理并检查
        if not torch.isfinite(image_seq).all():
            logger.warning("Vision sequence contains NaN/Inf! Sanitizing and Clamping.")
            # 1. 替换非有限值
            image_seq = torch.nan_to_num(image_seq, nan=0.0, posinf=1.0, neginf=-1.0)
            # 2. 限制极值范围，防止特征能量过大导致后续 LayerNorm 崩溃
            image_seq = torch.clamp(image_seq, min=-10.0, max=10.0)

        # 多模态嵌入处理
        multimodal_embedding = self.multimodal(data, image_embedding=image_seq)

        # 累加 MoE 专家负载均衡辅助损失（如果开启）
        try:
            proj = getattr(self.multimodal, "projection", None)
            if proj is not None and hasattr(proj, "get_aux_loss") and self.moe_aux_weight > 0.0:
                
                # 设置系数（可选）：平衡项、熵项、zloss 项
                if hasattr(proj, "set_aux_coefficients"):
                    proj.set_aux_coefficients(
                        balance=1.0,
                        entropy=self.moe_router_entropy_coef,
                        zloss=self.moe_router_zloss_coef,
                    )
                moe_aux = proj.get_aux_loss()
                # === 增加防御性检查 ===
                if torch.isfinite(moe_aux):
                    total_loss = total_loss + self.moe_aux_weight * moe_aux
                    out.update({"moe_aux_loss": moe_aux.detach()})
                else:
                    logger.warning("Detected Non-finite MoE aux loss, skipping this term.")
                total_loss = total_loss + self.moe_aux_weight * moe_aux
                out.update({"moe_aux_loss": moe_aux.detach()})
                # 记录门控统计（用于监控专家使用情况）
                if hasattr(proj, "get_gate_stats"):
                    stats = proj.get_gate_stats()
                    out.update({
                        "moe_importance": stats.get("importance"),
                        "moe_load": stats.get("load"),
                        "moe_entropy": stats.get("entropy"),
                        "moe_zloss": stats.get("zloss"),
                    })
        except Exception as e:
            logger.debug(f"MoE aux loss calculation failed: {e}")
            pass
    ...
    def segment(self,
                ...
                modal_inputs: Optional[Dict] = None,
                context: Optional[Dict[str, Union[bool, float, int]]] = None,
                threshold: float = 0.5) -> Dict[str, Union[torch.Tensor, np.ndarray]]:
        """
        使用视觉融合特征和 MOE 分割头进行分割推理。

        输入:
            images: [B, C, H, W] 图像张量
            input_size: (H, W) 原图尺寸；默认从 images 读取
            modal_inputs: 可选的模态输入（光学/SAR 等空间特征）
            context: 可选的上下文标志（如是否多云）
            threshold: 掩膜二值化阈值

        输出:
            dict 包含 logits、prob、mask（numpy）等
        """
        assert getattr(self, "seg_enabled", False) and hasattr(self, "seg_head"), "Segmentation head not enabled."
        if not isinstance(self.vision, DualVisionEncoder):
            raise RuntimeError("segment() 需要 DualVisionEncoder 以获取 fused_spatial。请在配置中开启 rgb_vision.arch=dual。")

        # 视觉编码（带空间特征）
        _, fused_spatial, _ = self.vision.encode_with_spatial(images)
        H, W = images.shape[-2], images.shape[-1]
        input_size = input_size or (H, W)
        context = context or {}

        seg_out = self.seg_head(fused_spatial, input_size=input_size, modal_inputs=modal_inputs, context=context)
        seg_logits = seg_out["logits"]  # [B, num_classes, H, W]
        seg_prob = torch.softmax(seg_logits.float(), dim=1)
        # 取前景类别（假定类别1为目标类）
        if seg_prob.shape[1] > 1:
            fg_prob = seg_prob[:, 1:2]
        else:
            fg_prob = torch.sigmoid(seg_logits)
        fg_mask = (fg_prob > threshold).float()
        return {
            "logits": seg_logits,
            "prob": fg_prob,
            "mask": fg_mask.squeeze().cpu().numpy(),
        }
    ...
    def chat_analyze(self,
                     images: torch.Tensor,
                     user_prompt: str,
                     compute_area: bool = True) -> Tuple[str, Optional[np.ndarray]]:
        """
        高层对话分析：支持在输入中包含 [SEG] 直接触发分割；否则进行文本生成，并在回复包含 [SEG] 时后触发分割。

        返回: (analysis_report, final_mask)
        """
        # 1) 若输入携带 [SEG]，直接触发分割
        if (self.seg_token_str in user_prompt) and getattr(self, "seg_enabled", False):
            seg = self.segment(images)
            final_mask = seg["mask"]
            report = "[SEG] 指令已执行。"
            # 面积估算（可选）
            try:
                gsd = getattr(self.config, "gsd", None)
                if compute_area and gsd is not None:
                    pixel_count = int(np.sum(final_mask))
                    area_sq_km = (pixel_count * gsd * gsd) / 1e6
                    report += f"\nDetected Area: {area_sq_km:.4f} km²"
            except Exception:
                pass
            return report, final_mask

        # 2) 正常文本生成
        # 编码图像（不池化），让语言模型插入 <image> 特征
        image_embedding = self.encode_image(images, pool=False)
        prompt = f"{DEFAULT_IMAGE_TOKEN}\n{user_prompt}"
        try:
            response_text = self.language.generate(image_embedding=image_embedding, prompt=prompt)
        except Exception:
            # 回退：若 generate 接口不支持 prompt，则构造最简输入 ids
            input_ids = None
            response_ids = self.language.generate(image_embedding=image_embedding, input_ids=input_ids)
            response_text = self.language.tokenizer.decode(response_ids[0], skip_special_tokens=True)

        final_mask = None
        analysis_report = response_text

        # 3) 在模型回复中检测 [SEG]，后触发分割
        if (self.seg_token_str in response_text) and getattr(self, "seg_enabled", False):
            seg = self.segment(images)
            final_mask = seg["mask"]
            try:
                gsd = getattr(self.config, "gsd", None)
                if compute_area and gsd is not None:
                    pixel_count = int(np.sum(final_mask))
                    area_sq_km = (pixel_count * gsd * gsd) / 1e6
                    analysis_report += f"\n[CoastGPT Physics Engine]\nDetected Area: {area_sq_km:.4f} km²"
            except Exception:
                pass
            # 去除标记以净化输出
            analysis_report = analysis_report.replace(self.seg_token_str, "")

        return analysis_report, final_mask
