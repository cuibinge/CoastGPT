...
def validate_epoch(model, dataloader, device, geo_encoder):
    ...
    with torch.no_grad(), tqdm(dataloader, unit="batch", desc="Validation") as tepoch:
        for images, coordinates in tepoch:
            ...
            val_total_loss = 0.3 * val_contrastive_loss.item() + 0.7 * val_lonlat_mae
            
            # 累计指标
            total_val_loss += val_total_loss
            total_val_contrastive_loss += val_contrastive_loss.item()
            total_val_lonlat_mae += val_lonlat_mae * batch_size
            total_val_batches += 1
    ...
    avg_val_lonlat_mae = total_val_lonlat_mae / total_samples
    
    return avg_val_loss, avg_val_contrastive_loss, avg_val_lonlat_mae
...
def train(model, train_dataloader, val_dataloader, num_epochs, device, geo_encoder, save_path=None):
    ...
    for epoch in range(num_epochs):
        ...
        val_total_loss, val_contrastive_loss, val_lonlat_mae = validate_epoch(
            model, 
            val_dataloader, 
            device, 
            geo_encoder
        ...
        if save_path is not None and val_total_loss < best_val_loss:
            ...
            torch.save({
                ...
                'corresponding_lonlat_mae': val_lonlat_mae
...
def main():
    ...
    print("\n启动训练+验证（Encoder-Decoder直接预测经纬度）...")
    history = train(
        model, 
        train_dataloader, 
        val_dataloader, 
