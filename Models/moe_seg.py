...
def _safe_bilinear_interpolate(x: torch.Tensor, size: Tuple[int, int]) -> torch.Tensor:
    """Ascend NPU does not support bilinear upsample on bfloat16 tensors."""
    orig_dtype = x.dtype
    needs_cast = x.device.type == "npu" and x.dtype == torch.bfloat16
    if needs_cast:
...
class SPPSelectorAdapter(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for l in self.levels:
            ...
            p = _safe_bilinear_interpolate(p, size=(h, w))
            pyramids.append(p)
...
class ContextExpert(nn.Module):
    ...
    def forward(self, x):
        ...
        g = _safe_bilinear_interpolate(g, size=(h, w))
...
class PromptConditioner(nn.Module):
    """Use task/element/physical prompts to modulate segmentation features."""
    ...
    def __init__(
        ...
        hidden_dim: int = 256,
        scale_gain: float = 0.25,
        bias_gain: float = 0.25,
        spatial_gain: float = 0.5,
        dropout: float = 0.0,
        use_task_text: bool = True,
        use_element_text: bool = True,
        use_physical_prompt: bool = True,
    ):
        super().__init__()
        hidden_dim = max(32, int(hidden_dim))
        self.use_task_text = bool(use_task_text)
        self.use_element_text = bool(use_element_text)
        self.use_physical_prompt = bool(use_physical_prompt)
        self.scale_gain = float(scale_gain)
        self.bias_gain = float(bias_gain)
        self.spatial_gain = float(spatial_gain)

        if text_embed_dim is not None:
            text_embed_dim = int(text_embed_dim)
            def _proj():
                return nn.Linear(text_embed_dim, hidden_dim)
        else:
            def _proj():
                return nn.LazyLinear(hidden_dim)

        self.task_proj = _proj()
        self.element_proj = _proj()
        self.physical_proj = _proj()
        self.task_norm = nn.LayerNorm(hidden_dim)
        self.element_norm = nn.LayerNorm(hidden_dim)
        self.physical_norm = nn.LayerNorm(hidden_dim)
        ...
        self.query_head = nn.Linear(hidden_dim, feat_dim)
    ...
    def _project(
        ...
        if (not enabled) or vector is None:
            return None
        return norm(proj(vector))

    def forward(
        self,
        task_vec: Optional[torch.Tensor] = None,
        element_vec: Optional[torch.Tensor] = None,
        physical_vec: Optional[torch.Tensor] = None,
    ) -> Optional[Dict[str, torch.Tensor]]:
        parts = []
        for vector, enabled, proj, norm in (
            (task_vec, self.use_task_text, self.task_proj, self.task_norm),
            (element_vec, self.use_element_text, self.element_proj, self.element_norm),
            (physical_vec, self.use_physical_prompt, self.physical_proj, self.physical_norm),
        ):
            part = self._project(vector, enabled, proj, norm)
            if part is not None:
                parts.append(part)
        if not parts:
            return None
        ...
        return {
            "scale": self.scale_gain * torch.tanh(self.scale_head(fused)),
            "bias": self.bias_gain * torch.tanh(self.bias_head(fused)),
            "query": self.query_head(fused),
            "prompt_norm": fused.norm(dim=-1).mean(),
        }
...
class AquacultureSegMOE(nn.Module):
    ...
    def __init__(
        ...
        text_embed_dim: Optional[int] = None,
        prompt_condition_cfg: Optional[Dict[str, Any]] = None,
    ):
        super().__init__()
        ...
        prompt_condition_cfg = prompt_condition_cfg or {}
        if bool(prompt_condition_cfg.get("enable", True)):
            self.prompt_conditioner = PromptConditioner(
                feat_dim=ch_in,
                text_embed_dim=text_embed_dim,
                hidden_dim=int(prompt_condition_cfg.get("hidden_dim", min(256, max(64, ch_in)))),
                scale_gain=float(prompt_condition_cfg.get("scale_gain", 0.25)),
                bias_gain=float(prompt_condition_cfg.get("bias_gain", 0.25)),
                spatial_gain=float(prompt_condition_cfg.get("spatial_gain", 0.5)),
                dropout=float(prompt_condition_cfg.get("dropout", 0.0)),
                use_task_text=bool(prompt_condition_cfg.get("use_task_text", True)),
                use_element_text=bool(prompt_condition_cfg.get("use_element_text", True)),
                use_physical_prompt=bool(prompt_condition_cfg.get("use_physical_prompt", True)),
            )
        else:
            self.prompt_conditioner = None
    ...
    def forward(
        ...
        physical_vec = self._pool_text_embeddings(physical_prompt_embs, text_mask=physical_prompt_mask)
        prompt_condition = None
        semantic_response = None
        if self.prompt_conditioner is not None:
            prompt_condition = self.prompt_conditioner(
                task_vec=task_vec,
                element_vec=element_vec,
                physical_vec=physical_vec,
            )
        if prompt_condition is not None:
            scale = prompt_condition["scale"].to(x_shared.dtype).unsqueeze(-1).unsqueeze(-1)
            bias = prompt_condition["bias"].to(x_shared.dtype).unsqueeze(-1).unsqueeze(-1)
            x_shared = x_shared * (1.0 + scale) + bias
            query_dir = F.normalize(prompt_condition["query"].float(), dim=-1, eps=1e-6).to(x_shared.dtype)
            feature_dir = F.normalize(x_shared.float(), dim=1, eps=1e-6).to(x_shared.dtype)
            semantic_response = (
                feature_dir * query_dir.unsqueeze(-1).unsqueeze(-1)
            ).sum(dim=1, keepdim=True).tanh()
            x_shared = x_shared + (
                self.prompt_conditioner.spatial_gain
                * semantic_response
                * query_dir.unsqueeze(-1).unsqueeze(-1)
            )
        ...
        H, W = input_size
        if (h, w) != (H, W):
            fused = _safe_bilinear_interpolate(fused, size=(H, W))
        ...
        if prompt_condition is not None:
            out["prompt_condition_norm"] = prompt_condition["prompt_norm"]
        if semantic_response is not None:
            out["semantic_spatial_response"] = semantic_response.abs().mean()
        ...
        if self.training:
            ...
            out["router_aux"] = {"entropy": entropy, "entropy_loss": -entropy, "zloss": zloss}
        return out


SemanticSegMOE = AquacultureSegMOE
