...
class DualVisionEncoder(nn.Module):
    ...
    def _enable_vit_rope(self, state: Dict[str, torch.Tensor]):
        ...
        try:
            ...
            def apply_rope(xq: torch.Tensor, xk: torch.Tensor) -> (torch.Tensor, torch.Tensor):
                ...
                pair_dim = head_dim // 2
                device = xq.device
                N_actual = xq.shape[2]  # use actual sequence length, not hardcoded 197
                pos = torch.arange(N_actual, device=device).float()  # [N]
                m = periods.numel()
    ...
    def _interpolate_pos_embed(self, H_new: int, W_new: int) -> torch.Tensor:
        """Interpolate ViT position embeddings to a new grid size.

        Args:
            H_new: target grid height in patches (image_h // patch_size)
            W_new: target grid width in patches (image_w // patch_size)
        Returns:
            Interpolated pos_embed of shape [1, 1 + H_new*W_new, embed_dim]
        """
        pe = self.global_encoder.pos_embed  # [1, 1 + H_old*W_old, D]
        if not isinstance(pe, (torch.nn.Parameter, torch.Tensor)):
            return pe
        if pe.dim() != 3:
            return pe

        D = pe.shape[-1]
        N_old = pe.shape[1] - 1  # exclude CLS token
        H_old = int(math.sqrt(N_old))
        if H_old * H_old != N_old:
            return pe  # non-square grid, skip interpolation

        if H_old == H_new and H_old == W_new:
            return pe  # same size, no interpolation needed
    ...
    def _patch_vit_pos_embed(self, H_patches: int, W_patches: int):
        """Temporarily patch ViT pos_embed for arbitrary input size. Returns (old_pe, old_img_size) for restore."""
        ge = self.global_encoder
        old_pe = ge.pos_embed
        old_img_size = getattr(ge.patch_embed, 'img_size', None)
        new_pe = self._interpolate_pos_embed(H_patches, W_patches)
        if new_pe is not old_pe:
            ge.pos_embed = nn.Parameter(new_pe.to(device=old_pe.device, dtype=old_pe.dtype))
        if hasattr(ge.patch_embed, 'img_size') and old_img_size is not None:
            ge.patch_embed.img_size = (H_patches * 16, W_patches * 16)
        return old_pe, old_img_size

    def _restore_vit_pos_embed(self, old_pe, old_img_size):
        """Restore ViT pos_embed after _patch_vit_pos_embed."""
        self.global_encoder.pos_embed = old_pe
        if old_img_size is not None and hasattr(self.global_encoder.patch_embed, 'img_size'):
            self.global_encoder.patch_embed.img_size = old_img_size
    ...
    def _get_global_grid(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        restore_state = None
        try:
            # Patch pos_embed so forward_features works at non-224 sizes
            restore_state = self._patch_vit_pos_embed(H_p, W_p)
            try:
            ...
            # Manual ViT forward with interpolated pos_embed
            x = x_in
            ...
            if hasattr(self.global_encoder, 'cls_token') and isinstance(self.global_encoder.cls_token, torch.nn.Parameter):
                ...
                added_cls = True
            # Use interpolated pos_embed if available (pos_embed was already patched above)
            ...
            if restore_state is not None:
                self._restore_vit_pos_embed(*restore_state)
    ...
    def encode_with_spatial(
        ...
        # Align spatial dims to patch_size=16 for ViT; no forced resize.
        # Training transforms already produce fixed-size inputs when needed.
        _, _, H, W = x.shape
        H = (H // 16) * 16
        W = (W // 16) * 16
        pixel_values = F.interpolate(x, size=(int(H), int(W)), mode="bilinear", align_corners=False)
