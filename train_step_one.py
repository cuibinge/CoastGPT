...
def build_ds_config(config: ml_collections.ConfigDict):
    ...
    if opt_lower == "adamw":
        ...
        ds_config = {
            ...
            "fp16": {
                "enabled": True if config.fp16 else False,  # 是否启用 FP16 混合精度训练
                "auto_cast": False,  # 是否自动进行类型转换
                "initial_scale_power": 16,  # 初始缩放因子
                "loss_scale_window": 500,  # 损失缩放窗口
            },
            "bf16": {
                "enabled": True if config.bf16 else False,  # 是否启用 BF16 混合精度训练
                "auto_cast": False,  # 是否自动进行类型转换
            },
            "zero_optimization": {
                "stage": 2,  # ZeRO 优化阶段
...
def parse_option():
    ...
    # 基本配置参数
    parser.add_argument("--batch-size", type=int, help="batch size for single GPU")  # 单 GPU 批量大小
    ...
    # WandB 配置参数
    parser.add_argument("--wandb", type=str2bool, default=False, help="wandb logger")  # 是否使用 WandB 日志记录
    parser.add_argument("--entity", type=str, default="pumpkinn", help="wandb entity")  # WandB 实体名称
    parser.add_argument(
    ...
    parser.add_argument("--name", type=str, default="first_run", help="wandb run name")  # WandB 运行名称
    parser.add_argument("--notes", type=str, default=None, help="wandb run's notes")  # WandB 运行备注
    ...
    parser.add_argument("--local_rank", type=int)  # 本地进程排名
